...
#define DATABASE_URL "https://ses-project-2c8b9-default-rtdb.firebaseio.com"
//-----------------------------------------------------------------------
String room_no = "";
String room_no2 = "";
...
void setup() {
  ...
  Serial.println();
  String room_no = WiFi.localIP().toString();
  Serial.println(room_no);
  String room_no2 = "room"+ room_no;
  room_no2.replace(".", "");
  Serial.println(room_no2);
  //-----------------------------------------------------------------------
...
#endif
  //-----------------------------------------------------------------------
  if (!Firebase.RTDB.beginStream(&stream, room_no2))
...
void loop() {
  ...
  if (Firebase.ready() && signupOK) {
    if (uploadBucket == true) {
      uploadBucket = false;
      String URL = room_no2 + "/" + bucketPath;
      Serial.println(URL);
